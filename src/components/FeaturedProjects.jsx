                ...
                <article className="grid flex-1 gap-10 py-10 lg:grid-cols-[1.08fr_0.92fr] lg:items-start lg:py-14">
                    <section>
                        ...
                        <motion.div className="mt-10 overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.03] shadow-glow" initial={{ scale: 0.98 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 260, damping: 24 }}>
                            <motion.img layoutId={`project-${project.id}`} className="h-auto max-h-[390px] w-full object-cover" src={image} alt={`${project.title} project preview`} decoding="async" />
                        </motion.div>
                    </section>
                    ...
                    <aside className="lg:pt-24">
                        ...
                        <div className="mt-10 flex flex-col gap-3 sm:flex-row lg:flex-col">
                            {project.links.live && (
                                <a href={project.links.live} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full bg-emerald-300 px-5 py-3 text-sm font-bold text-[#031008] transition hover:bg-emerald-200 focus:outline-none focus:ring-2 focus:ring-emerald-300 focus:ring-offset-2 focus:ring-offset-[#030504]">
...
export default function FeaturedProjects({ projects = projectData }) {
    ...
    return (
        ...
        <section id="projects" className="relative isolate overflow-hidden px-5 py-16 sm:px-8 sm:py-20 lg:px-20 lg:py-24">
            ...
            <div className="mx-auto max-w-8xl">
                        ...
                        <div className="space-y-6 sm:space-y-6">
                            {projects.map((project) => {
                                ...
                                return (
                                    <motion.article
                                        key={project.id}
                                        className="group grid gap-6 sm:grid-cols-[3.2rem_minmax(0,1fr)] sm:gap-9"
                                    ...
                                    >
                                        ...
                                        <div className="relative max-w-xl pb-1 pt-3">
                                            ...
                                            <button type="button" onFocus={() => setHoveredId(project.id)} onBlur={() => setHoveredId(null)} onClick={(event) => openProject(project, event.currentTarget)} className="mt-5 inline-flex items-center gap-4 text-lg font-medium text-emerald-300 transition hover:gap-5 hover:text-emerald-200 focus:outline-none focus:ring-2 focus:ring-emerald-300 focus:ring-offset-4 focus:ring-offset-[#020403]">
                                            ...
                                            {isHovered && (
                                                <div className="mt-7 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] transition duration-300 sm:hidden">
                                                    <motion.img layoutId={`project-${project.id}`} src={image} alt={`${project.title} project preview`} className="aspect-[16/10] w-full object-cover" loading="lazy" decoding="async"/>
                                                </div>
                                            )}
                ...
                <div className="mt-[130px] flex items-center justify-center gap-4 pl-0 text-white/40 sm:pl-[5.7rem]">
